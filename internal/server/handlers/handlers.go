...
func (h *Handlers) HandlePostUserRegister(next http.Handler) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !(u.User =="" || u.Password=="") {
			http.Error(w, "user or password is empty", http.StatusBadRequest)
			return
		 }

		//Check if username exists
		 userChk, err := h.Storage.GetUser(r.Context(), u.User)
		if userChk != nil {
			//login has already been occupied
			http.Error(w, "login "+userChk.User+" is occupied", 409)
			return
		}
		err = h.Storage.SaveUser(r.Context(), *u)
